...
if [[ $(cat package.json | grep "name" | sed s'/[",]//g' | awk '{print $2}') == node-IDLL ]]; then
	...
	echo "$showinfo We are inside a ${yellow}Node-IDLL$stand Folder"
	...
	if [[ ! $(cat package.json | grep "name" | sed s'/[",]//g' | awk '{print $2}') == node-IDLL ]]; then
		...
		echo -e "---Choose where to run PM2 instance\\n${cyan}$(ls -d -1 "$PWD"/** | grep 'Node-IDLL')$stand\\n---"

		read -r -e -p "$showinput Enter the full location of the Node-IDLL folder where you want to start the pm2 instance: " nodeIDLLloc

		if [[ -d $nodeIDLLloc ]]; then

			if cd "$nodeIDLLloc"; then echo "$showinfo Changing DIR to $yellow$(pwd)$stand"; else echo "$showerror Couldn't change DIR to $nodeIDLLloc"; fi
			echo "$showinfo Folder location changed to $nodeIDLLloc"
...
if [[ $(cat package.json | grep "name" | sed s'/[",]//g' | awk '{print $2}') == node-IDLL ]]; then
	...
	if [[ ! $(cat package.json | grep "name" | sed s'/[",]//g' | awk '{print $2}') == node-IDLL ]]; then
		...
		read -r -e -p "$showinput Enter the full location of the Node-IDLL folder where you want to start the SCREEN instance: " nodeIDLLloc
