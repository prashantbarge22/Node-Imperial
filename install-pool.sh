...
if [[ $(cat package.json | grep "name" | sed s'/[",]//g' | awk '{print $2}') == node-IDLL ]]; then
	...
	elif cat /etc/*release | grep -q -o -m 1 centos; then if [[ -d $HOME/.nvm ]]; then echo "$showok NVM is already installed!"; elif [[ ! -d $HOME/.nvm ]]; then curl -o- https://raw.githubusercontent.com/creationix/nvm/v0.33.11/install.sh | bash && export NVM_DIR="$HOME/.nvm" && [ -s "$NVM_DIR/nvm.sh" ] && \. "$NVM_DIR/nvm.sh" && [ -s "$NVM_DIR/bash_completion" ] && \. "$NVM_DIR/bash_completion" && source ~/.bash_profile && nvm install 16 && nvm use 16 && nvm alias default 16; fi fi
	...
	if [[ $(ls -d vue-Frontend) == vue-Frontend ]]; then
		...
		function vue_upd() {
			...
			if [[ "$yn_update" == [nN] ]]; then
				...
				if cd vue-Frontend; then echo "$showexecute Changing dir to ${yellow}vue-Frontend$stand"; else echo "$showerror Couldn't cd to vue-Frontend folder!"; fi
                        	...
                        	echo "$showexecute Going back to Node-IDLL folder..." && if cd ..; then echo "$showexecute Changing dir to ${yellow}$(pwd)$stand"; else echo "$showerror Couldn't cd back!"; fi
		...
		vue_upd
	        ...
	        if [[ ! $(ls -d vue-Frontend) == vue-Frontend ]]; then
			echo "$showerror vue-Frontend not found inside Node-IDLL!"
			echo "$showinfo Cloning vue-Frontend from IDLL repository..."
			...
			if [[ $(cat package.json | grep "name" | sed s'/[",]//g' | awk '{print $2}') == node-IDLL ]]; then
				echo "$showinfo We're inside a Node-IDLL folder!"
	...
	echo "$showerror You are not inside the Node-IDLL folder."
	echo "$showinfo To install your POOL, use this script inside the Node-IDLL folder."
fi
