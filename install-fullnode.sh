...
echo -e "-----\\n${white}1. First time config - Create a IDLL Full Node\\n2. Deploy clean IDLL Full Nodes\\n3. Clone IDLL Node \\n4. Exit$stand"

read -r -e -p "$showinput Choose option: " readft ### Ask if this is the first time node configuration

if [[ "$readft" == 1 ]]; then

echo "$redhashtag If you will use more than one PORT (80, 8080 etc), please note that it's better to create multiple Node-IDLL folders. $redhashtag"
...
read -r -e -p "$showinput How many nodes do you want to deploy? " readnrofnodes ### How many nodes will the user deploy
	...
	if [[ "$readnrofnodes" == 0 ]]; then
		...
		echo "$showerror You can't deploy 0 IDLL Nodes."
		ftconfig
		...
		do
			if [[ ! -d Node-IDLL1 ]]; then
				git clone https://github.com/IDLL/Node-IDLL.git Node-IDLL"$nodes"
				cd Node-IDLL"$nodes" && npm install && cd ..
				echo "$showok Node-IDLL$nodes Deployed successfully!"
			else
				echo "$showwarning Node-IDLL1 is already deployed!"
			fi
		done
...
echo "$showinfo Now you may run sudo bash node-start.sh"
	...
	if [[ "$readdeploy" =~ ^(y|yes|Y|YES|Yes)$ ]]; then
		...
		if [[ "$readnrofnodesdeploy" =~ ^[[:digit:]]+$ ]]; then
			...
			crnt_dir=$(pwd)
			countcurrentnodes=$(sudo find / -name "Node-IDLL[0-9]" | wc -l)
#			countcurrentnodes="7"  # this is for testing purposes only

			if [[ $(grep "name" package.json | sed s'/[",]//g' | awk '{print $2}') == node-IDLL ]]; then

				echo "$showinfo We are inside a Node-IDLL Folder"
				cd .. && echo "$showinfo Location changed to $(pwd)"
				for ((i=countcurrentnodes+1; i<=countcurrentnodes+readnrofnodesdeploy; i++));
				do
					git clone https://github.com/IDLL/Node-IDLL.git Node-IDLL"$i"
					cd Node-IDLL"$i" && npm install && cd ..
					echo "$showok Node-IDLL$i Deployed successfully!"
				done
			else
				if [[ ! $(grep "name" package.json | sed s'/[",]//g' | awk '{print $2}') == node-IDLL ]]; then
					echo "$showok We are outside of a Node-IDLL Folder"

					for ((i=countcurrentnodes+1; i<=countcurrentnodes+readnrofnodesdeploy; i++));
					do
						git clone https://github.com/IDLL/Node-IDLL.git Node-IDLL"$i"
						cd Node-IDLL"$i" && npm install && if cd "$crnt_dir"; then echo "$showinfo Current DIR changed to $(pwd)"; else echo "$showerror Couldn't change DIR to $crnt_dir!"; fi
						echo "$showok Node-IDLL$i Deployed successfully!"
					done
				fi
			fi
	...
	function f_cloneIDLLnode(){

	read -r -e -p "$showinfo Would you like to Clone a IDLL Node?(y or n) " cloneIDLLnode

	if [[ "$cloneIDLLnode" =~ ^(y|yes|Y|YES|Yes)$ ]]; then

		read -r -e -p "$showinfo How many Nodes do you want to Clone? " deployclones

		echo "$showinfo Proceeding with IDLL Node Cloning so you can use it on another $showport"
...
#			countcurrentnodes="7"  # this is for testing purposes only
			...
			if [[ $(grep "name" package.json | sed s'/[",]//g' | awk '{print $2}') == node-IDLL ]]; then
				...
				read -r -e -p "$showinput Enter the full location for the Node-IDLL you want to clone: " nodeIDLLloc

				if [[ -d $nodeIDLLloc ]]; then

					echo "$showinfo Entered folder location $showok"

					for ((i=countcurrentnodes+1; i<=countcurrentnodes+deployclones; i++));
					do
						cp -r "$nodeIDLLloc" Node-IDLL"$i"
						ls -la Node-IDLL"$i"
						echo "$showok Node-IDLL$i Deployed successfully @ $nodeIDLLloc/Node-IDLL$i!"
					done
				else
					if [[ ! -d $nodeIDLLloc ]]; then
	...
	elif [[ "$cloneIDLLnode" =~ ^(n|no|N|NO|No)$ ]]; then

		echo "$showinfo Ok. Bye."
		exit 0

	elif [[ "$cloneIDLLnode" == "" ]]; then
	...
	f_cloneIDLLnode # node-IDLL clone function
