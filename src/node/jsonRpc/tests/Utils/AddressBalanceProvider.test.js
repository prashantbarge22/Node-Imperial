...
describe('AddressBalanceProviderTest', () => {
    ...
    it ('should throw exception if address is invalid', () => {
        sinon.stub(AccountantTree, 'getBalance').withArgs('IDLL$invalid').returns(null);

        const oAddressBalanceProvider = new AddressBalanceProvider(AccountantTree);
        expect(() => {oAddressBalanceProvider.getBalance({address: 'IDLL$invalid'});}).to.throw('Address is invalid');
    });

    it ('should return the current balance if address is valid', () => {
        sinon.stub(AccountantTree, 'getBalance').withArgs('IDLL$valid').returns(100);

        const oAddressBalanceProvider = new AddressBalanceProvider(AccountantTree);
        assert.strictEqual(oAddressBalanceProvider.getBalance({address: 'IDLL$valid'}), 100);
    });
