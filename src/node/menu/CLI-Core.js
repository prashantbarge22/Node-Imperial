...
export class CLICore {
        ...
        return new Promise(async (resolve) => {
            ...
            for (let i = 0; i < Blockchain.Wallet.addresses.length; i++)
                if (addressString === Blockchain.Wallet.addresses[i].address)
                    answer = await Blockchain.Transactions.wizard.validateTransaction(Blockchain.Wallet.addresses[i].address, toAddress, amountToSend * IDLLCoins.IDLL, feeToSend, undefined, undefined, timelock - 1, nonce, true);
    ...
    async listAddresses() {
        ...
        for (let i = 0; i < Blockchain.Wallet.addresses.length; ++i) {
            ...
            balance = (balance === null) ? 0 : (balance / IDLLCoins.IDLL);
    ...
    exportAddress(id) {
        ...
        return new Promise(async (resolve) => {
            ...
            let addressString = Blockchain.Wallet.addresses[addressId].address;
            let fileName = "IDLL$" + Blockchain.Wallet.addresses[addressId].unencodedAddress.toString("hex") + ".IDLL";
