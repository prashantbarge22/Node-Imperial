...
describe('MiniBlockchainAccountantTree', () => {
    ...
    it('save MiniBlockchainAccountantTree Tree', async () => {
        ...
        let array = [
                     {addr: "IDLL$gCnD#W5qC6g#u0XD$8orGEhTYoCx+VDEH$$", val: "000661817095001" },
                     {addr: "IDLL$gAePSrtnvtesE6#xeKsvV4dkFbjGI6Q9kD$", val: 124213},
                     {addr: "IDLL$gDojfYVXudrLz$XHyJQtFIYxBeS+fNQaoL$", val: 123233 },
                     {addr: "IDLL$gBzsiV+$FARK8qSGqs09V6AEDBi#@fP6n7$", val: 15323313}
            ];
        ...
        assert(!Tree.root.hash.equals( Buffer.alloc(32)), "root hash is not valid "+Tree.root.hash.toString("hex"));
    ...
    it('save MiniBlockchainAccountantTree Tree multiple tests', async () => {
        ...
        for (let i = 0; i < addresses.length; i++){
            list[i] = {
                address: addresses[i],
                value: TestsHelper.makeRandomNumber(IDLLCoins.MAX_SAFE_COINS / NUMBERS, false) + 1,
            }
        }
    ...
    it('save MiniBlockchainAccountantTree Tree multiple tests with + - ', async () => {
        ...
        for (let i = 0; i < addresses.length; i++){
            ...
            let value = TestsHelper.makeRandomNumber(IDLLCoins.MAX_SAFE_COINS / (NUMBERS*10), false) + 1;
            list[i] = {
                address: addresses[i],
                value1: value,
                value2: 0,
    ...
    it('save MiniBlockchainAccountantTree Tree multiple tests with + - NONCES', async () => {
        ...
        for (let i = 0; i < addresses.length; i++){
            ...
            let value = TestsHelper.makeRandomNumber(IDLLCoins.MAX_SAFE_COINS / (NUMBERS*100), false) + 1;
        ...
        for (let i = 0; i < NUMBER_TESTS; i++){
            ...
            for (let i = 0; i < tests; i++){
                ...
                switch (test){
                    ...
                    case 0:
                        value = Math.floor( Math.random() * IDLLCoins.MAX_SAFE_COINS / 100 );
                        break;

                    case 1:
                        nonce += Math.floor( Math.random() * IDLLCoins.MAX_SAFE_COINS / 100 );
                        break;
    ...
    it('MiniBlockchainAccountantTree Tree test symmetric using lexicographic comparisons ', async () => {
        ...
        for (let i = 0; i < NUMBER_TESTS; i++) {
            ...
            let value = Math.floor( Math.random() * IDLLCoins.MAX_SAFE_COINS / 1000 );
        ...
        assert(!Tree1.root.hash.equals(Buffer.alloc(32)), "root hash is not valid "+Tree1.root.hash.toString("hex"));
