...
class InterfaceMerkleTreeNode extends InterfaceTreeNode{
    ...
    _getValueToHash(){
        ...
        if (Buffer.isBuffer(this.value) )
            ...
            return IDLLCryptoData.createIDLLCryptoData(this.value, true).buffer;
    }
    ...
    _computeHash(){
        ...
        if (this.edges.length === 0){ //Leaf Node (terminal node)
            ...
            this.hash = IDLLCrypto.SHA256( IDLLCrypto.SHA256( valueToHash ) )
        ...
        if (this.edges.length > 0){
            ...
            this.hash = IDLLCrypto.SHA256( IDLLCrypto.SHA256( Buffer.concat ( [valueToHash, hashConcat]  ) ));
