...
class MiniBlockchainBlock extends inheritBlockchainBlock {
    ...
    async computeHashPOS( newTimestamp, posNewMinerAddress, balance ){
        ...
        try {
            ...
            let hash = await IDLLCrypto.SHA256(buffer);
            ...
            if (balance === null || balance < consts.BLOCKCHAIN.POS.MINIMUM_AMOUNT * IDLLCoins.IDLL)
    ...
    calculateChainHash(){
        ...
        if ( BlockchainGenesis.isPoSActivated(this.height) )
            return IDLLCrypto.SHA256( IDLLCrypto.SHA256( this._calculateSerializedBlock( true ) ));
        else
