...
import IDLLCryptoData from "common/crypto/IDLL-Crypto-Data";
...
class InterfaceBlockchainTransaction{
    ...
    _computeTxId(){
        return IDLLCrypto.SHA256( IDLLCrypto.SHA256( this.serializeTransaction() ));
    }
    ...
    validateTransactionOnce( blockHeight, blockValidationType = {} ){
        ...
        if (!IDLLCoins.validateCoinsNumber(inputSum))
            throw {message: "Transaction inputSum is invalid", inputSum: inputSum};

        if (!IDLLCoins.validateCoinsNumber(outputSum))
    ...
    deserializeTransaction(buffer, offset=0, returnTransaction=false){
        ...
        if (!Buffer.isBuffer(buffer))
            buffer = IDLLCryptoData.createIDLLCryptoData(buffer).buffer;
    ...
    _processTransactionFees(){
        ...
        if (!IDLLCoins.validateCoinsNumber(diffInFees))
