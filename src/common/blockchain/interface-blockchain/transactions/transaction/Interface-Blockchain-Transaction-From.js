...
class InterfaceBlockchainTransactionFrom {
    ...
    setFrom(addresses, currencyTokenId) {
        ...
        if ( !currencyTokenId ) {
            currencyTokenId = Buffer.alloc(consts.MINI_BLOCKCHAIN.TOKENS.IDLL_TOKEN.LENGTH);
            currencyTokenId[0] = consts.MINI_BLOCKCHAIN.TOKENS.IDLL_TOKEN.VALUE;
        }
    ...
    validateFrom() {
        ...
        if ( !(this.currencyTokenId.length === consts.MINI_BLOCKCHAIN.TOKENS.IDLL_TOKEN.LENGTH || this.currencyTokenId.length === consts.MINI_BLOCKCHAIN.TOKENS.OTHER_TOKENS.LENGTH))
        ...
        for (let fromObject of this.addresses ){
            ...
            if (!IDLLCoins.validateCoinsNumber(fromObject.amount))
