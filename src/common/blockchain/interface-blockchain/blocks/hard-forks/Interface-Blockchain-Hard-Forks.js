...
class InterfaceBlockchainHardForks{
        ...
        for (let i=height; i<this.blockchain.blocks.length; i++) {
            ...
            for (let j = 0; j < block.data.transactions.transactions.length; j++) {
                ...
                if (addrIndex !== -1) {
                    ...
                    console.log("from ",from," => ", to, transaction.to.addresses[0].amount/IDLLCoins.IDLL, " :: ", transaction.from.addresses[0].amount/IDLLCoins.IDLL);
                    ...
                    console.log("miner  ", miner, "  ", fee / IDLLCoins.IDLL);
    ...
    async revertAllTransactions(address, height, revertActions, totalAmount, finalAddress){
        ...
        for (let key in this._output)
            console.log(key, this.blockchain.accountantTree.getBalance(key) / IDLLCoins.IDLL, "IDll");
        ...
        console.log("DONE", this.blockchain.accountantTree.getBalance( finalAddress )/IDLLCoins.IDLL, "IDLL");
