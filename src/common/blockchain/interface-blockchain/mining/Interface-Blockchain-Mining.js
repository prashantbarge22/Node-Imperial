...
class InterfaceBlockchainMining extends  InterfaceBlockchainMiningBasic{
    ...
    async mineBlock( block,  difficulty, start, end, height, showLogsOnlyOnce){
        ...
        try{
            ...
            if (answer && answer.result && this.blockchain.blocks.length === block.height ){
                ...
                console.warn( "----------------------------------------------------------------------------");
                console.warn( "IDLL Block was mined ", block.height ," nonce (", answer.nonce+")", "timestamp", block.timeStamp, answer.hash.toString("hex"), " reward", (block.reward / IDLLCoins.IDLL), "IDLL", block.data.minerAddress.toString("hex"));
    ...
    async _minePOS(block, difficultyTarget){
        ...
        if ( !balance || balance < consts.BLOCKCHAIN.POS.MINIMUM_AMOUNT * IDLLCoins.IDLL ){

            await this.blockchain.sleep( Blockchain.MinerPoolManagement.minerPoolStarted ? 10000 : 1000 );

            console.warn("Not enough funds to mine POS. It is required "+consts.BLOCKCHAIN.POS.MINIMUM_AMOUNT + " IDLL to mine POS");
