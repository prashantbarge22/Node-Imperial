...
import IDLLCrypto from "../../../crypto/IDLL-Crypto";
...
class PoolSettings {
    ...
    constructor(wallet, poolManagement, databaseName){
        ...
        this._poolPrivateKey = IDLLCrypto.getBufferRandomValues(64);
  ...
  printPoolSettings(showPoolPrivateKey=false){
    ...
    _generatePoolURL(){
        ...
        this.poolURL =  ( process.env.BROWSER ? window.location.origin : 'http://IDLL.ddns.net:9094' ) +'/pool/1/'+encodeURI(poolName)+"/"+encodeURI(this.poolFee)+"/"+encodeURI(this.poolPublicKey.toString("hex"))+"/"+encodeURI(servers);
    ...
    async _getPoolPrivateKey(){
        ...
        if ( !this._poolPrivateKey ) {
            ...
            // let finalPrivateKey = Buffer.concat( [ IDLLCrypto.SHA256(IDLLCrypto.MD5(privateKey)), IDLLCrypto.SHA256( IDLLCrypto.RIPEMD160(privateKey) )]);
            //
