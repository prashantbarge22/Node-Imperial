...
const PAYOUT_FEE = IDLLCoins.IDLL * consts.MINING_POOL.POOL_PAYOUT_FEE;
...
class PoolRewardsManagement{
    ...
    createRemainPayouts(){
        ...
        Log.info("I have to pay "+remainingAmount/IDLLCoins.IDLL+" IDLL from " + poolCurrentBalance/IDLLCoins.IDLL, Log.LOG_TYPE.POOLS);
        ...
        this.poolData.miners.forEach((miner)=>{
            ...
            if ( reward >= consts.MINING_POOL.MINING.MINING_POOL_MINIMUM_PAYOUT){
                ...
                //Log.info("Will pay " + reward/IDLLCoins.IDLL.toFixed(0) + " IDLL to " + InterfaceBlockchainAddressHelper.generateAddressWIF(miner.address,false,true), Log.LOG_TYPE.POOLS);
        ...
        Log.info("Total to pay " + sumTotal/IDLLCoins.IDLL.toFixed(0), Log.LOG_TYPE.POOLS );
    ...
    async createTransactions(){
        ...
        for (let i=this._toAddresses.length-1; i>=0; i--){
            ...
            if (this._toAddresses[i].amount < consts.MINING_POOL.MINING.MINING_POOL_MINIMUM_PAYOUT){
                ...
                Log.info("Will pay " + this._toAddresses[i].amount / IDLLCoins.IDLL + " IDLL to " + InterfaceBlockchainAddressHelper.generateAddressWIF(this._toAddresses[i].address, false, true), Log.LOG_TYPE.POOLS);
