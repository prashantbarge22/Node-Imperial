...
import IDLLCoins from "../../../utils/coins/IDLL-Coins";
...
class MinerPoolProtocol extends PoolProtocolList{
    ...
    async _sendPoolHello(socket){
        ...
        try{
            ...
            let message = IDLLCrypto.getBufferRandomValues(32);
    ...
    async pushWork( miningAnswer, poolSocket){
        ...
        try {
            ...
            if (miningAnswer.pos)
                Log.info( "timestamp " + miningAnswer.pos.timestamp + "   " + "balance " + (Blockchain.AccountantTree.getBalance( Blockchain.blockchain.mining.minerAddress  ) / IDLLCoins.IDLL), Log.LOG_TYPE.POOLS);
